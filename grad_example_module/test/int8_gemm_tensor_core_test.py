...
def main():
  ...
  for problem in problems:
    ...
    for conf in tqdm(all_cases, total=num_cases):
      ...
      exec_name = f'int8_batched_gemm_tensor_core_kernels/simt_igemm_batched_{tb_shape[0]}x{tb_shape[1]}x{tb_shape[2]}_{mma_shape[0]}x{mma_shape[1]}x{mma_shape[2]}_{inst_shape[0]}x{inst_shape[1]}x{inst_shape[2]}_test'
      ...
      if not os.path.isfile(exec_name):
        ...
        compile_ret = subprocess.run(f"nvcc -gencode=arch=compute_80,code=compute_80 -I/home/beomsik/dp/cutlass/include test_main.cu -o {exec_name}", shell=True, capture_output=True)
