...
__global__ void compute_scaling_factor_cuda_kernel(float *out, 
                                                   ...
                                                   float max_norm,
                                                   int scale_loss) {
  *(out + threadIdx.x) = min((max_norm / (*(norm + threadIdx.x) + 1e-6)), 1.0) / scale_loss;
}
...
void compute_scaling_factor_cuda(float *out, 
    ...
    compute_scaling_factor_cuda_kernel<<<1, num_rows_to_compute>>>(out, norm, max_norm, scale_loss);
}
