...
class Trainer(transformers.Trainer):
    ...
    def train(self, model_path=None, dev_objective=None, dev_objective_key=None, privacy_engine=None):
        """
        ...
        for epoch in train_iterator:
            ...
            with BatchMemoryManager(
                data_loader=epoch_iterator, max_physical_batch_size=self.privacy_args.real_per_device_train_batch_size, optimizer=optimizer
                ) as new_train_loader:
                for step, inputs in tqdm(enumerate(new_train_loader), desc="Iteration", disable=False, total=len(new_train_loader)):
                    ...
                    if inputs["input_ids"].shape[0] != self.privacy_args.real_per_device_train_batch_size:
                        continue

                    labels = inputs["labels"]
                    losses, inputs = self.training_step(model, inputs)
                    tr_loss += losses["scalar_loss"]

                    if (step + 1) % (self.args.per_device_train_batch_size / self.privacy_args.real_per_device_train_batch_size) == 0 or (
                        ...
                        self.global_step += 1
                        self.epoch = epoch + (step + 1) / len(new_train_loader)
                    ...
                    #         self.optimizer.step(loss=vector_loss, callback=callback)
                        ...
                        scheduler.step()
                        # optimizer.step()
                        optimizer.step(input = inputs, criterion = criterion_func_non_reduction, target=labels)
                    ...
                    #     self.epoch = epoch + (step + 1) / len(epoch_iterator)
                        ...
                        if not self.privacy_args.non_private:
                            ...
                            # print(inputs)
                            # optimizer.step()
                            optimizer.step(input = inputs, criterion = criterion_func_non_reduction, target=labels)  # noqa
                            optimizer.zero_grad()
                        # if (
                        #     self.args.evaluation_strategy in (IntervalStrategy.STEPS, EvaluationStrategy.STEPS) and
                        #     self.global_step % self.args.eval_steps == 0
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> dict:
        ...
        return dict(vector_loss=vector_loss, scalar_loss=scalar_loss), inputs
    ...
    def evaluate_and_log(
        ...
        with open(os.path.join(self.args.output_dir, "accuracy_vs_time.csv"), "a+") as f:
            f.write(f"{get_elapsed_time()},{metrics['eval_acc']}\n")
        ...
        if not self.privacy_args.non_private:
            ...
            # logs["privacy_spent"] = privacy_engine.get_privacy_spent(accounting_mode="all", lenient=True)
            logs["privacy_spent"] = privacy_engine.get_epsilon()
