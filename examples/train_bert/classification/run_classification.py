...
from opacus import config as opacus_config
...
def main():
    ...
    model.roberta.embeddings.position_embeddings.requires_grad_(False)
    model.roberta.embeddings.token_type_embeddings.requires_grad_(False)
    ...
    if privacy_args.non_private:
        ...
        #     quant_mode=privacy_args.quant_mode,
        #     skip_checks=True
        # )
        opacus_config.benchmark = False
        opacus_config.model_type = "transformer"
        if "base" in model_args.model_name_or_path:
            opacus_config.architecture = "bert-base"
        elif "large" in model_args.model_name_or_path:
            opacus_config.architecture = "bert-large"
        if privacy_args.dpsgd_mode == "naive":
            opacus_config.dpsgd_mode = opacus_config.MODE_NAIVE
        elif privacy_args.dpsgd_mode == "reweight":
            opacus_config.dpsgd_mode = opacus_config.MODE_REWEIGHT
        elif privacy_args.dpsgd_mode == "elegant":
            opacus_config.dpsgd_mode = opacus_config.MODE_ELEGANT
        else:
            raise ValueError
        ...
        # Set dummy data loader to set privacy engine
        input_ids = torch.randint(0, 20000, (data_args.max_seq_length,)).type(torch.long)
        attention_mask = torch.randint(1, 2, (data_args.max_seq_length,)).type(torch.long)
        token_type_ids = torch.randint(0, 1, (data_args.max_seq_length,)).type(torch.long)

        input_ids = input_ids.expand(
                    1, total_train_batch_size, data_args.max_seq_length
                    ).reshape((1) * total_train_batch_size, data_args.max_seq_length)
        ...
        labels = torch.ones(total_train_batch_size, dtype=torch.int64).repeat(1)
        dummy_train_dataset = TensorDataset(*inputs, labels)
        train_loader = torch.utils.data.DataLoader(
            dummy_train_dataset,
            batch_size=total_train_batch_size,
        )
        sample_rate = total_train_batch_size / len(train_dataset)
        if privacy_args.target_delta is None:
            privacy_args.target_delta = len(train_dataset) ** -1.1

        model.train()
        trainer.model, trainer.optimizer, _ = privacy_engine.make_private_with_epsilon(
            module=model,
    ...
    if training_args.do_train:
        ...
        # Don't reload.
        trainer.train(model_path=None, privacy_engine=privacy_engine)
    ...
    if training_args.do_eval:
        ...
        print(f"Epsilon : {privacy_engine.get_epsilon(delta=privacy_args.target_delta)}, Delta : {privacy_args.target_delta}", flush=True)
    ...
    print(f"Total elapsed : {end - start} sec", flush=True)
