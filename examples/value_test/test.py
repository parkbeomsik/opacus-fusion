...
def test(model_type, architecture, dpsgd_mode, grad_sample_mode):
    ...
    execute_command(f"python benchmark.py --input_size 32 --architecture {architecture} --model_type {model_type} --dpsgd_mode naive --batch_size 16 --model_load_path value_test/value_data/{architecture}_weight.pt --input_load_path value_test/value_data/{architecture}_input.pt --grad_save_path value_test/value_data/{architecture}_grad_{dpsgd_mode}_{grad_sample_mode}.pt --profile_value -c 0.1 --sigma 0.0 --grad_sample_mode ew")
