...
for case in experiments:
    ...
    args = f"nice -n 10 taskset -c 0-20 python benchmark.py --warm_up_steps 0 --steps 1 --input_size {input_size} --architecture {architecture} --model_type {model_type} --dpsgd_mode {algo} {'--quant' if quant else ''} --batch_size {batch_size} --profile_memory --log_file {LOG_FILE_NAME}"
    print(args)
