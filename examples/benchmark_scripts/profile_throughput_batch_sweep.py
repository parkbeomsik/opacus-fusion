...
# Set environment
LOG_FILE_NAME="results/v100_throughput_batch_sweep.csv"
...
for experiment in experiments:
    ...
    if algo == "sgd":
        ...
        dpsgd_mode = f"--dpsgd_mode {algo} --adaptive_clipping"
    else:
    ...
    return_code = 0
    while return_code == 0:
        args = f"nice -n 10 taskset -c 0-20 python benchmark.py --steps 20 --input_size {input_size} --architecture {architecture} --model_type {model_type} {dpsgd_mode} --grad_sample_mode {grad_sample_mode} --batch_size {batch_size} --profile_throughput --log_file {LOG_FILE_NAME}"
